...
public class Tpm {
    ...
    @RequiresPermissions("quote:create")
    public TpmQuoteResponse tpmQuote(TpmQuoteRequest tpmQuoteRequest, @Context HttpServletRequest request) throws IOException, TAException {
        ...
        try {
            com.intel.mtwilson.core.tpm.Tpm tpm = com.intel.mtwilson.core.tpm.Tpm.open(Paths.get(Folders.application(), "bin"));
            assetTagHash = tpm.readAssetTag(ownerAuth);
            if (assetTagHash.length != 0) {
        ...
        } catch (com.intel.mtwilson.core.tpm.Tpm.TpmException ex) {
