...
public class TrustagentConfiguration {
    ...
    public final static String SECURE_STORE_PASSWORD_ALIAS = "secure_store_password_alias";
    ...
    public String[] getTrustagentTlsCertDnsArray() throws SocketException {
        ...
        HashSet<String> uniqueHostNames = new HashSet();
        uniqueHostNames.addAll(TlsCertDNsList);
        String[] dnListArray = new String[uniqueHostNames.size()];
        if (dnListArray.length > 0) {
            log.debug("Retrieved Domain Names from network configuration: {}", (Object[])dnListArray);
            return uniqueHostNames.toArray(dnListArray);
        }
	...
	public String getTrustagentSecureStorePassword() {
        return conf.get(TRUSTAGENT_SECURESTORE_PASSWORD, null); // intentionally no default - this must be generated during setup
    }
    ...
    public static TrustagentConfiguration loadConfiguration() throws IOException {
        ...
        TrustagentConfiguration configuration;
        if( file.exists()) {
            String content;
		...
		guardedPassword.setPassword(Environment.get(PASSWORD));
				...
				guardedPassword.setPassword(IOUtils.toString(passwordIn).trim());
                        }
                    }
                }
                Configuration  configurationProvider = new EncryptedConfigurationProvider(new FileResource(file), guardedPassword.getInsPassword()).load();
		guardedPassword.dispose();
                return new TrustagentConfiguration(configurationProvider);
                } catch (Exception e) {
                  Logger.getLogger(TrustagentConfiguration.class.getName()).log(Level.SEVERE, null, e); 
                }
            }
            Properties properties = new Properties();
            ...
            configuration = new TrustagentConfiguration(new PropertiesConfiguration(properties));
            return configuration;
        } 
        else {
            configuration = new TrustagentConfiguration(new PropertiesConfiguration());
            return configuration;
        }
    }
