#!/bin/bash
TBOOT_VERSION="1.9.7"
TBOOTXM_VERSION="4.6.1-SNAPSHOT"
...
maven_get_tboot() {
  ...
  # UBUNTU
    ...
    echo "Unsupported OS: Cannot retrieve tboot-dist package"
    exit 1
  fi
  #_destination="$(pwd)/${_artifact_id}-${_version}-${_classifier}.${_packaging}"
  _destination="$(pwd)/${_artifact_id}-${_version}"
  ...
  if [ $? -ne 0 ]; then echo "WARNING: cannot retrieve tboot-dist RPM package via maven"; fi
  maven_get "${_group_id}" "${_artifact_id}" "${_version}" "deb" "amd64" "${_destination}-amd64.deb"
  if [ $? -ne 0 ]; then echo "WARNING: cannot retrieve tboot-dist DEB package via maven"; fi
}
