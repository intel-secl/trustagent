...
IF DEFINED MTWILSON_API_URL (IF DEFINED MTWILSON_TLS_CERT_SHA256 (IF DEFINED MTWILSON_API_USERNAME (IF DEFINED MTWILSON_API_PASSWORD (IF "%PROVISION_ATTESTATION%"=="y" (call "%trustagent_cmd%" provision-attestation))))) ELSE (Call :tee Trust agent setup: Attestation service details unavailable. Skipping provisioning)
...
IF [%AUTOMATIC_FLAVOR_CREATION%] == [""] set AUTOMATIC_FLAVOR_CREATION=
IF "%AUTOMATIC_FLAVOR_CREATION%"=="y" (
    call "%trustagent_cmd%" create-host-unique-flavor
)
