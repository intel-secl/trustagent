...
var /Global MTWILSON_TLS_CERT_SHA384
...
LangString CIT_SUBTITLE ${LANG_ENGLISH} "Checking installation of prerequisites for Intel Trust Agent"
...
LangString ENV_SUBTITLE ${LANG_ENGLISH} "Host Verification Service and Trust Agent environment settings"
...
Function .onInit
        ...
        prereq:
                ...
                StrCpy "$INIFILE" "$EXEDIR\trustagent.ini"
                ...
                StrCpy $2 "Name like '%%Microsoft Visual C++ 2017 x64 Minimum Runtime%%'"
                nsExec::ExecToStack 'wmic product where "$2" get name'
                Pop $0
                Pop $1
                ${StrStr} $0 $1 "Microsoft Visual C++ 2017 x64 Minimum Runtime"
                StrCmp $0 "" notfound
                     StrCpy $vcr1Flag 1
                     Goto done1
                notfound:
...
Function CITServerPage
        ...
        Pop $mylabel
        ${if} $vcr1Flag == 1
               ${NSD_CreateLabel} 0 40 100% 12u "Microsoft Visual C++ 2017 Redistributable x64 is installed"
               Pop $mylabel
        ${else}
                  MessageBox MB_OK "Microsoft Visual C++ not installed properly. Exiting the Trust Agent Installation.."
                  Abort
        ${endif}
        ${NSD_CreateLabel} 0 100 100% 12u "Please ensure that Intel Host Verification Service is running for Trust Agent."
        Pop $mylabel
        nsDialogs::Show
FunctionEnd
